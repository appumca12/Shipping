...
Public Class UserBlsCntrs
    ...
    Public Sub SetDocument(ByVal BID As Guid, ByVal ds As DataSet)
        ...
        CntrsDS.Tables("CntrsCharges").Clear()
    ...
    Private Sub GridCntrs_BeforeExitEdit(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.BeforeExitEditModeEventArgs) Handles GridCntrs.BeforeExitEditMode
        ...
        If GridCntrs.ActiveCell.Column.Header.Caption = "CntrNo" Then
            ...
            Else
                ...
                'appu - Empty B/L Creation Modification Request

                'sql = " select top 1 * from TB_EcsMoves where CntrID = (select ID from TB_EcsContainers where CntrNo = '" & GridCntrs.ActiveCell.Text & "') and Status = 'EL' order by StatusDate desc"
                Dim TbName As String
                TbName = IIf(IOStatus = InwardOutward.Inward, "TB_Inward", "TB_Outward") & TB.TableName
                If IOStatus = InwardOutward.Outward Then

                    sql = " select top 1 * from [TB_EcsContainers] where cntrno = '" & GridCntrs.ActiveCell.Text & "' and  LastStatus in ('EL','MB')"
                    TableBySql(sql, TB, JResult)
                    If TB.Rows.Count > 0 Then
                        GridCntrs.ActiveRow.Cells("FLE").Value = "E"
                        GridCntrs.ActiveRow.Cells("SOC").Value = "N"
                        GridCntrs.ActiveRow.Cells("NoOfPkgs").Value = 1
                        GridCntrs.ActiveRow.Cells("PackageType").Value = "VAN"
                    End If
                End If
            End If
        End If
        If GridCntrs.ActiveCell.Column.Header.Caption = "GW" Then
            If IOStatus = InwardOutward.Outward Then
                Dim sql As String
                Dim TB As DataTable
                Dim JResult As Boolean
                sql = " select top 1 * from [TB_EcsBank] where Cntrno = '" & GridCntrs.ActiveRow.Cells("CntrNo").Value & "'"
                TableBySql(Sql, TB, JResult)
                If TB.Rows.Count > 0 Then
                    Try
                        If Not IsDBNull(TB.Rows(0).Item("Payload")) Or Not TB.Rows(0).Item("Payload").ToString() = "0" Then
                            If Convert.ToDouble(GridCntrs.ActiveCell.Text.Replace("_", "")) > Convert.ToDouble(TB.Rows(0).Item("Payload")) Then
                                MessageBox.Show("GW more than Payload. Please input valid GW", "Shipping")
                                'MsgBox("GW more than Payload!. Proceed?", MsgBoxStyle.YesNo + MsgBoxStyle.Information)
                                GridCntrs.ActiveCell.Value = 0.0
    ...
    Private Sub ImportsFromEcsMrtCntrs()
        ...
        TableBySql("Select *  , " & _
                   ...
                   " from Tb_EcsContainers Ecs Where OutVoyageID = '" & Cmb.VoyageID.ToString & "' and LastStatus in ('EL','MB')", TB, lResult)
