...
Public Class FrmStatestical
    ...
    Private Sub MenuItem_CustomPickupClick() Handles MenuItem.CustomPickupClick
        ...
        'Txt = "List Of B/LS From " & Format(Me.dFrom.DateValue, "Short date") & "  To:" & Format(Me.dTo.DateValue, "Short date")
        'ReformatGrid(GridDetails, Txt)
    ...
    Private Sub RowToColumnVoyage(ByVal TB As DataTable)
        ...
        Tmp.Columns.Add(New DataColumn("Arrival", System.Type.GetType("System.DateTime")))
        Tmp.Columns.Add(New DataColumn("BLS", System.Type.GetType("System.Int32")))
        Tmp.Columns.Add(New DataColumn("POD"))
    ...
    Private Sub RowToColumnPorts(ByVal TB As DataTable)
        ...
        For Each Dr In TB.Rows
            Drs = Tmp.Select("POL = '" & Dr("POL") & "' And POD = '" & Dr("POD") & "'")
            If Drs.Length = 0 Then
                NewDr = Tmp.NewRow
                NewDr("POL") = Dr("POL")
                NewDr("POD") = Dr("POD")
        ...
        Try
            ...
            If Me.GridPorts.Rows.Count > 0 Then
                Select Case Me.TabControl.SelectedTab.Key
                    Case "Ports"
                        SaveGridToExcel(GridPorts)
                    Case "Detention"
                        SaveGridToExcel(GridDetention)
                    Case "Voyage"
                        SaveGridToExcel(GridVoyage)
                    Case "Clients"
                        SaveGridToExcel(GridClients)
                    Case "Details"
                        SaveGridToExcel(GridDetails)
                End Select
