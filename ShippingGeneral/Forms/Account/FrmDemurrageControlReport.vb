...
Public Class FrmDemurrageControlReport
    ...
    Private Sub ButtonPickup_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonPickup.Click
        If Me.TabControl.SelectedTab.Text = "Demurrage Control Report" Then
            PickupMovement()
        End If

    End Sub
    Private Sub ButtonMail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonMail.Click
        If Me.TabControl.SelectedTab.Text = "Demurrage Control Report" Then
            SendMail()
        End If
    ...
    Private Sub SendMail()
        ...
        If Me.MoveDateFrom.DateValue Is Nothing Or Me.MoveDateTo.DateValue Is Nothing Then
            MessageBox.Show("Please Pick From and To Date.")
            Exit Sub
        End If
        ...
        Try
            If System.IO.File.Exists(MailFile) = True Then
                ' System.IO.File.Delete(MailFile)
            Else
                xlWorkSheet.SaveAs(MailFile)
                xlWorkBook.Close()
                xlApp.Quit()
                DA.Dispose()
        ...
        Dim cntrmaxdate As DateTime
        cntrmaxdate = DateTime.MaxValue
        cntrmaxdate = Convert.ToDateTime((TB.Compute("max(AllContainerReturnBackDate)", String.Empty)))

        Dim cntrmindate As DateTime
        cntrmindate = Convert.ToDateTime((TB.Compute("min(AllContainerReturnBackDate)", String.Empty)))
        MailUser.lblFrom.Text = cntrmindate.ToString("MM/dd/yyyy HH:mm:ss")
        MailUser.lblTo.Text = cntrmaxdate.ToString("MM/dd/yyyy HH:mm:ss")
        MailUser.Show()
    ...
    Private Sub PickupMovement()
        ...
        dFrom = DateTime.MaxValue
        Dim dTo As DateTime
        dTo = DateTime.MaxValue
        Dim RL As DataRelation
        Dim PCol(0) As DataColumn
        Dim cCol(0) As DataColumn
        Dim DS2 As DataSet
        ...
        End If
        dFrom = Me.MoveDateFrom.DateValue
        Dim DfromTime As String
        ' DfromTime = dFrom.ToString("MM/dd/yyyy") + " 00:00:00"
        'dFrom = DateTime.Parse(DfromTime)
        dFrom = Me.MoveDateFrom.DateValue
        ...
        Cmd = New SqlCommand("SP_DemurageControlReport")
        ...
        For Each Dr In TB.Rows
            ...
            '    a = 0
            'End If
            ' MessageBox.Show(Dr("BLNO").ToString())
            ...
            For Each Dr1 In TbSum.Rows
                ...
                Else
                    Dr("Detention Total Amount") = 0
                    Dr("Total Paid") = 0
                    Dr("Balance Amount") = 0
                    Dr("Detention TotalAmount USD") = 0
                    Dr("TotalPaid USD") = 0
                    Dr("BalanceAmount USD") = 0

                End If
            Next
            Dr("Detention Total Amount") = Invoice
            Dr("Total Paid") = TotalPaid
            Dr("Detention TotalAmount USD") = Invoice_USD
            Dr("TotalPaid USD") = TotalPaid_USD
            'appu - tap entry check
            sSql = "Select * From TB_TAPDetention Where Blno ='" & Dr("BLNO") & "'"
            TableBySql(sSql, TB1, lResult)
            If TB1.Rows.Count > 0 Then
                Dr("Balance Amount") = 0
                Dr("BalanceAmount USD") = 0
        ...
        '.Layout.Rows.Band.Columns("DocumentCategories").ButtonDisplayStyle = UltraWinGrid.ButtonDisplayStyle.Always
        ...
        With Me.GridSummary.DisplayLayout.Bands(0)
            ...
            .Header.Caption = "Demurrage Contro Report:" & dFrom & "  To:" & dTo
        ...
        GridNumSeperator(Me.GridSummary)
        Dim path As String = Directory.GetParent(Directory.GetCurrentDirectory()).ToString()
        Dim MailFilen As String
        MailFilen = path + "\DemurageReport.xlsx"

        System.GC.Collect()
        System.GC.WaitForPendingFinalizers()
        Dim fs As System.IO.FileStream
        If System.IO.File.Exists(MailFilen) = True Then
            fs = New System.IO.FileStream(MailFilen, IO.FileMode.Open, IO.FileAccess.Read)
            fs.Dispose()
            fs.Close()
        End If
        ...
        Try
            If System.IO.File.Exists("c:\TEMP\a.xlsx") = True Then
                System.IO.File.Delete("c:\TEMP\a.xlsx")
            End If
            If System.IO.File.Exists(MailFilen) = True Then
                System.IO.File.Move(MailFilen, "c:\TEMP\a.xlsx")
    ...
    End Sub
    Private Sub GridSummary_ClickCellButton(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles GridSummary.ClickCellButton

        FrmInwardInvoice.TextBlNo.TextValue = Me.GridSummary.ActiveRow.Cells("BlNo").Value
        'FrmInwardInvoice.ShowDialog(Me)
        FrmInwardInvoice.ShowDialog()
        FrmInwardInvoice.Close()
    End Sub
    Private Sub GridSummary_InitializeRow(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.InitializeRowEventArgs) Handles GridSummary.InitializeRow
        e.Row.Cells("View Invoice").Value = "Invoice"
        'e.Row.Cells("View Invoice").Appearance.ForeColor = Color.Blue
        'e.Row.Cells("View Invoice")
    End Sub
    ...
    'End Sub
    Private Sub FrmDemurrageControlReport_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub saveExcelFile(ByVal FileName As String)
        ...
        'Dim SmtpServer As New SmtpClient("136.243.19.209")
        'mail.From = New MailAddress("shipping@iranlandsea.com")
        ''Dim SmtpServer As New SmtpClient("smtp.gmail.com")
        ''mail.From = New MailAddress("smappubca@gmail.com")
        'mail.[To].Add("ilsmuthu@gmail.com")
        'mail.[CC].Add("appumca12@gmail.com")
        'mail.Subject = "BL - All Containers Returned from " + dFrom.ToString() + " to " + dTo.ToString()
        'mail.Body = "PFA Demurage BL list"
        ...
        'SmtpServer.Port = 25 ' SMTP Email Server port
        'SmtpServer.Credentials = New System.Net.NetworkCredential("shipping@iranlandsea.com", "ship@123")
