...
Public Class FrmEcs
    'Private ItemLevel As String = "201" commented by thenmozhi after checking with DB
    ...
    Private isMoveValid As Boolean = True
    ...
    Private Sub SetDocument()
        ...
        'appu - TW from inwardCntrs
        Dim sql As String
        Dim sql2 As String
        Dim TB As DataTable
        Dim TB2 As DataTable
        Dim JResult As Boolean
        Dim JResult2 As Boolean
        If IsDBNull(Dr("TW")) Or Dr("TW").ToString() = "0" Or Dr("TW").ToString() = "" Then
            'sql = " select TW from [TB_InwardCntrs] C, TB_InwardBLS B where C.cntrno = '" & Dr("CntrNo").ToString() & "' and B.id = C.BLID  and B.VoyageID ='" & Dr("InVoyageID").ToString() & "'"
            sql = " select TW from [TB_ECSBank] where cntrno = '" & Dr("CntrNo").ToString() & "'"
            TableBySql(sql, TB, JResult)
            If TB.Rows.Count > 0
                If IsDBNull(TB.Rows(0).Item("TW")) Or TB.Rows(0).Item("TW").ToString() = "" Then
                    Me.TWNo.GnrlNum.Value = IIf(IsDBNull(Dr("TW")), String.Empty, Dr("TW"))
                Else
                    sql2 = " select TW from [TB_InwardCntrs] C, TB_InwardBLS B where C.cntrno = '" & Dr("CntrNo").ToString() & "' and B.id = C.BLID  and B.VoyageID ='" & Dr("InVoyageID").ToString() & "'"
                    TableBySql(sql2, TB2, JResult2)
                    If TB2.Rows.Count > 0 Then
                        Me.TWNo.GnrlNum.Value = Val(TB2.Rows(0).Item("TW"))
                    Else
                        Me.TWNo.GnrlNum.Value = Val(TB.Rows(0).Item("TW"))
                    End If
                End If
            Else
                Me.TWNo.GnrlNum.Value = IIf(IsDBNull(Dr("TW")), String.Empty, Dr("TW"))
    ...
    Private Sub GridMoves_AfterCellUpdate(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles GridMoves.AfterCellUpdate
        ...
        Try
            If Me.GridMoves.ActiveCell.Column.Key = "StatusDate" Then
                ...
                End If
                If DateDiff(DateInterval.Day, Me.GridMoves.ActiveRow.Cells("StatusDate").Value, Date.Today) > 2 And Not (CurrentUser.UserCode = "LH" Or CurrentUser.UserCode = "MK") Then
                    MessageBox.Show("Status Date cannot update after 48hrs. Please contact Admin!", "ILS Shipping")
                    isMoveValid = False
    ...
    Private Sub SaveDocument()
        ...
        Try
            'appu
            If IsDBNull(Me.RepairCost2.GnrlNum.Value) Then
                Me.RepairCost2.GnrlNum.Value = "0.00"
            End If

            If IsDBNull(Me.InvoiceAmount2.GnrlNum.Value) Then
                Me.InvoiceAmount2.GnrlNum.Value = "0.00"
            End If
     ...
     MessageBoxDefaultButton.Button1)
            ...
            'If Not IsDBNull(Me.RepairCost2.GnrlNum.Value) Then
            ...
            'If Not IsDBNull(Me.InvoiceAmount2.GnrlNum.Value) Then
            If Me.InvoiceAmount2.GnrlNum.Value > 0 And Me.InvCurrency2.Code = "" And Not IsNothing(Me.InvoiceAmount2.GnrlNum.Value) Then
    ...
    Public Sub GeneralForms_CustomSaveClick(ByVal sender As Object, ByVal formId As Integer)
        If (UserGeneralForms.getItemLevel(UserGeneralForms.UserTabSelected) = Convert.ToInt32(ItemLevel)) Then 'For ILS Shortcuts
            If ValidateData() = True And isMoveValid = True Then
        ...
        If (UserGeneralForms.getItemLevel(UserGeneralForms.UserTabSelected) = Convert.ToInt32(ItemLevel)) Then 'For ILS Shortcuts
            ...
            If MsgBoxDelete() = Windows.Forms.DialogResult.Yes Then
                ...
                Try
                    ...
                    dm.CommitAll()
                    GenerateLog(LogDataSet)
                    Me.GeneralForms_CustomFindClick(sender, formId)
                    Me.EcsTab.Tabs(0).Selected = True
