...
Public Class CLGenerateDPA
        ...
        For Each Dr In BLS
            ...
            For Each Cn In Cns
                ...
                With DPACTR
                    ...
                    '.H_CHECKDIGIT = Right(Cn.CntrNo, 1) :  by appu
                    .H_CHECKDIGIT = "1"
                ...
                With DPACON
                    ...
                    .H_CONSIGNMENTPACKAGES = Cn.NoOFPkgs
                    .H_PACKAGETYPE = Pkgs(0).MEANING
                    .H_PACKAGETYPECODE = Pkgs(0).STDCode
    ...
    End Sub
    Public Sub GenerateDPA_New(ByVal VoyageID As String, ByVal Port As String, ByVal BLS() As DataRow, ByVal FleName As String)
        ' appu - pakgtype
        Dim SW As New StreamWriter(FleName)
        ...
        Dim BL As TB_InwardBLS
        Dim Cn As TB_InwardCntrs
        Dim CnPkg As TB_InwardCnsPkgs
        Dim CnPkgCol As TB_InwardCnsPkgsCollection
        Dim Cns As TB_InwardCntrsCollection
        Dim Goods As TB_InwardBlsGoods
        Dim Names As TB_InwardBlsNames
        Dim Lines As TB_Lines
        ...
        For Each Dr In BLS
            ...
            For Each Cn In Cns
                ...
                DM.QueryCriteria.Clear()
                DM.QueryCriteria.And(JoinPath.TB_InwardCnsPkgs.Columns.CntrID, Cn.ID, MatchType.Exact)
                CnPkgCol = DM.GetTB_InwardCnsPkgsCollection
                Dim serial As Integer
                serial = 0
                For Each CnPkg In CnPkgCol
                   
                    With DPACON
                        serial += 1
                        .H_SERIALNUMBER = serial
                        .H_CARGODESCRIPTION = CnPkg.EMSNumber
                        .H_COMMODITYCODE = IIf(CnPkg.IMDGCode = "", "1", CnPkg.IMDGCode) '"1"
                        .H_CONSIGNMENTPACKAGES = CnPkg.NoOFPkgs
                        Dim TB As New DataTable
                        Dim sSql As String
                        Dim lResult As Boolean
                        sSql = "select top 1 Meaning from TB_CODE where Groupcode = 'PG' and STDCode = '" & Trim(CnPkg.PackageType) & "'"
                        TableBySql(sSql, TB, lResult)
                        If TB.Rows.Count > 0 Then
                            .H_PACKAGETYPE = TB.Rows(0).Item(0).ToString
                        End If

                        .H_PACKAGETYPECODE = CnPkg.PackageType
                        .H_CONSIGNMENTWEIGHTIN = CnPkg.GW 'Goods.TGW
                        .H_DANGEROUSGOODS = "N"
                        .H_REFRIGERATIONREQUIRED = "N"
                    End With
                    SW.WriteLine(DPACON.GenerateSegmentCON)
