...
Public Class CLInwardManifest
    ...
    Private TBBlList As DataTable
    Dim TBlsList As New DataTable
    ...
    Public Sub GenrateMnf(ByVal SelTB As DataTable, ByVal Voy As TB_Voyage)
        ...
        tempDs = StoredProcedures.SP_InwardCntrsPkgsTotal(Voy.ID.ToString)
        ...
        Ds.Tables.Add(TempBlList.Copy) 'Added for TOP URGENT \ INWARD GENERAL LIST Case
        '   Ds.WriteXml("C:\Mnfst.xml")
        Ds = New DataSet
        ...
        With Cmd
            ...
            .CommandText = "SP_RptInwardMnfSummary"
        ...
        Ds = New DataSet
        'FillDatasetByCmd(Cmd, Ds, "TempSummary", lResult) ' For TOP URGENT \ INWARD GENERAL LIST uncommented
        Ds.WriteXml("C:\Temp\Bls.xml")
    ...
    Public Sub GenrateMnf(ByVal Line As String, ByVal BlNo As String)
        ...
        Call InitDs()
        Dm = New DataManager(My.Settings.DSN)
        Dm.QueryCriteria.And(JoinPath.TB_InwardBLS.Columns.Line, Line, MatchType.Exact)
    ...
    Private Sub GenMnfBl(ByVal Carrier As String, ByVal BLNo As String)
        ...
        Dim Imdg As TB_InwardCnsImcoCollection
        Dim ImdgTemp As TB_InwardCnsImcoCollection
        Dim CnsTotal As String

        Dim TempRefs As TB_InwardCnsTempratureCollection
        Dim TempRef As TB_InwardCnsTempratureCollection
        ...
        If Dm.GetTB_InwardBLSCollectionFromDataSet.Count > 0 Then
            ...
            End If
            Imdg = Dm.GetTB_InwardCnsImcoCollectionFromDataSet
            TempRefs = Dm.GetTB_InwardCnsTempratureCollectionFromDataSet
        End If
        Memo = New CLMemo
        ...
        End If
        General.InwardCntrsCount(Bls.ID, CnsTotal)
        If Not Goods Is Nothing Then
            Memo.ColInit(CLMemo.MemoTypes.Goods, CnsTotal & " " & vbCrLf & " " & Goods.Goods, 35)
        ...
        If Not Memo.Shpr Is Nothing Then
            For nI = 1 To Memo.Shpr.Count
                If Len(Trim(Memo.Shpr.Item(nI))) > 0 Then
                    ...
                    End If
                    Dr("Shipment") = GetMeaningCode("ST", Bls.BLStatus)
                    TempMnf.Rows.Add(Dr)
                End If
        ...
        Else
            Dr("Col3") = Bls.BlNo
            Dr("Col6") = "(TW)"
            If Goods.TTW.ToString = "Null" Then
                Dr("Col7") = ""
            Else
                Dr("Col7") = Format(Double.Parse(Goods.TTW + 0), "standard")
            End If
        ...
        TableBySql("SP_InwardTotalBlChargesByBLID '" & Bls.ID.ToString & "'", TB, lResult)


        If TBChg Is Nothing Then
            TBChg = TB ' Dm.DataSet.Tables("TB_InwardCharges")
        Else
            'TBChg.Merge(Dm.DataSet.Tables("TB_InwardCharges"))
            TBChg.Merge(TB)
        End If
        'Below code added for TOP URGENT \ INWARD GENERAL LIST Case
        TableBySql("SP_InwardBlsListByVoyageID '" & CurrentShippingLine & "', '" & voy.ID.ToString & "'", TBlsList, lResult)
        TempBlList = TBlsList
        'End If
    End Sub
    Private Sub InitDs()
        ...
        With TempMnf
            ...
            myDataColumn = New DataColumn
            myDataColumn.DataType = System.Type.GetType("System.String")
            myDataColumn.ColumnName = "Shipment"
            .Columns.Add(myDataColumn)
    ...
    Public Sub ShowSummary(ByVal SelTB As DataTable, ByVal Vessel As String, ByVal Voyage As String)
        ...
        'Dim Report As RptInwardSumm = New RptInwardSumm
        'Dim lResult As Boolean
    ...
    Public Sub MnfCover(ByVal Voy As TB_Voyage, ByVal VesselName As String)
        ...
        sSql = "SP_InwardCntrsByVoyage '" & Voys.ID.ToString & "'"

        Dm = New DataManager(My.Settings.DSN)
        DS = StoredProcedures.SP_InwardCntrsByVoyage(Voys.ID.ToString)
        ...
        For Each Dr In DS.Tables("Cover").Rows
            ...
            aKey(0) = NullToValue(Dr("StatusCode"), 3)
